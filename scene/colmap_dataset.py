...
class Colmap_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        """
        frame_paths = glob.glob(os.path.join(self.root_dir, "frame*"))
        frame_paths = sorted(frame_paths)

        self.N_frames =   self.final_frame - self.first_frame 
    ...
    def load_images_path(self,frames_paths):
        ...
        for index, frame_path in enumerate(frames_paths):
            index = int(frame_path.split('/')[-1].split('frame')[-1])
    ...
    def __getitem__(self,index):
        ...
        return img, self.image_poses[index], self.image_times[index], self.image_paths[index] 
...
class Colmap_Dataset_from_Blender(Dataset):
    ...
    def load_images_path(self):
        ...
        for i in range(self.N_frames):   # 300 # final_frame - first_frame
            image_poses.append(self.poses[i])
            image_times.append(i/self.N_frames)
    ...
    def __getitem__(self,index):
        ...
        if self.downsample != 1.0:
            img = img.resize(self.img_whs_cps[index], Image.LANCZOS)
        img = self.transform(img)   # toTensor
