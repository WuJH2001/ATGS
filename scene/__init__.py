...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, frames_start_end=[0,300], load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, args.lod, frames_start_end, llffhold=cfg.llffhold)
            dataset_type = "colmap"
        ...
        if cfg.open_sync_time:
            self.gaussians.load_sync_file(os.path.join(args.source_path, 'optimized', 'sync.json'))

        if self.loaded_iter:
            self.gaussians.spatial_lr_scale = self.cameras_extent
            cloud_dir = os.path.join(self.model_path, "point_cloud", "iteration_" + str(self.loaded_iter))
            self.gaussians.load_ply_sparse_gaussian(os.path.join(cloud_dir, "point_cloud.ply"),
                                                   scene_info.point_cloud, frames_start_end)
            self.gaussians.load_model(cloud_dir)
        else:
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent, self.maxtime, frames_start_end)


    def save(self, iteration, dir_suffix=""):
        point_cloud_path = os.path.join(self.model_path, f"point_cloud{dir_suffix}/iteration_{iteration}")
        os.makedirs(point_cloud_path, exist_ok=True)
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        self.gaussians.save_mlp_checkpoints(point_cloud_path)

        if cfg.save_optimizer:
            self.gaussians.save_optimizer(point_cloud_path)
