...
def generateBasketballCameras(poses, width, height, FovX, FovY, cam):
    ...
    times = [i/total_frames for i in range(total_frames)]
    # times = [0.1]
    for idx, w2c in enumerate(w2cs):
        R = w2c[:3, :3].transpose()
        T = w2c[:3, 3]

        cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY , FovX=FovX , image=cam.image, time= times[idx%total_frames], mask = None,
...
def interpolation_frames(train_cam_infos, frames_start_end, train_dataset):
    ...
    gop_size = frames_start_end[1] - frames_start_end[0]
    for cam in train_cam_infos[::gop_size]:
    ...
    if cfg.render_kind == 'smooth':
        virtual_poses = interpolate_virtual_poses_smooth(train_poses, int(cfg.virtual_frame_interval))
    else:
        virtual_poses = interpolate_virtual_poses_sequential(train_poses, int(cfg.virtual_frame_interval))
